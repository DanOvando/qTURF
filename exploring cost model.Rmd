...
out$cost_derby = (a*q*out$pop)/(2*out$effort)

out$cost_itq = 0.77 * out$cost_derby 

# out$cost_itq = (a*q)/2

out$catch = pmin(out$pop,out$pop*q*out$effort)

dat <- out %>%
  as_data_frame() %>%
  gather('rights','cost',cost_derby:cost_itq) %>%
    mutate(catch = pmin(pop,pop*q*effort), revenue = catch*a,
         costs = cost*effort^2,
         profits = revenue - costs,
         marginal_prof = a*q*pop - 2*(cost*effort))

  dat %>%
  ggplot(aes(effort,cost, fill = rights)) +
  geom_point(shape = 21) +
